...
int main(int argc, const char * argv[]){
    ...
    for(int i = 0; i < 5000; i++){
        fscanf(ifp, "%d;%d;%d;%d;%d;%d;%d",
            &dataArray[7*i],&dataArray[(7*i)+1],&dataArray[(7*i)+2],&dataArray[(7*i)+3],&dataArray[(7*i)+4],&dataArray[(7*i)+5],&dataArray[(7*i)+6],&dataArray[(7*i)+7]);
    }

    group* shots;
    int shots_len;

    downscaleAndGroupData(dataArray, 5000, shots, &shots_len);
...
void downscaleAndGroupData(int* data, int n_data, group* shots, int* shots_len) {
    ...
    _groupData(average, n_data, &shots, shots_len);
}
...
void _groupData(datapoint* data, int n_data, group** groups, int* groups_len) {
    ...
    *groups = malloc(sizeof(group)*(n_data/10));
    datapoint* currentGroupEnd = data;

    for(k = 0; k < n_data; k++){
        if(measuring){
            (*currentGroupEnd) = data[k];
            currentGroupEnd++;
            g.length++;
        }
        if(((data[k].RX < 5000) || (data[k].RX > -5000)) ? 1 : 0){
            zeroStreak++;
            if(zeroStreak == 10){
                (*groups)[*groups_len] = g;
                (*groups_len)++;
